...
The availability of industrial machinery  is crucial to any business operating in the manufacturing sector. Mechanical failures can halt production and unplanned downtime is disruptive and costly. Small failures can lead to catastrophic failures which exponentially increases downtime and repair costs. Identifying a degradation condition before reaching catastrophic failure is key to increasing and maintaining machine availability. For these reasons a large field of academic work is dedicated to analyzing the health of a machine, predicting it's remaining life and thus preventing these kind of failures.
...
A number of linear regression models are created based on the results from the correlated features as well as some multivariate models. These are then compared using a FIT metric. Potentially clustering of machine states are highlighted based on visual observations of the feature combinations. The conclusions drawn from this study include identification of relationships between signals, potential non-linear correlations and suggestions for future data collection.
\clearpage

\section{Acknowledgments}
I would like to thank my supervisors Daniel Ronnow and Oscar B for their guidance and patience.
I would like to thank the University of Gavle for offering a Masters program available to take as distance education.
\newpage
...
The data for this project comes from a straightening machine at a multinational steel manufacturer. It consists of one hour of data over a 15 day period. It is assumed that it was taken from the same period of time each day. The data acquisition system used to obtain this data is a Iba-DAQ acquisition system with a sampling time 0.02 seconds. 
...
The list of signals given by the company are given in Table \ref{signalNames}.
\begin{center}
\begin{tabular}{ |c|c|l| }
 \hline
...
\end{center}
\subsection{Matlab}
Matlab's Diagnostic Feature Designer was used calculate the features from the signal pulse data with Table \ref{signalNames} showing the features that were extracted. For the remainder of the report some figures are labeled with a feature number instead of the full text due to space constraints. Refer to Table \ref{signalNames} to find which feature each index represents.
\begin{center}
\begin{tabular}{ |c|l| }
 \hline
 1 & Clearance Factor \\
...
State detection for this study is performed using signal 5, 21:20 Actual moment under Rolls. The data is separated into cycles using this signal to isolate just the periods where the machine is in the act or processing a part. A threshold of 600 was used as the threshold and a threshold of X samples was also used to filter out short pulses which aren't believed to be full cycles.
