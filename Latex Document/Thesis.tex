...
The application of CM and predictive maintenance (\gls{PM}) is a large area of research in the machine and manufacturing industry. Of course it is not only limited to machines being utilized on other engineering structures like bridges, large process plants such as waster water and also biological systems. 
% Discuss more specifically bearing vibration analysis
In the manufacturing sector, a common application of (\gls{CM}) is the vibration analysis of rotating machines. The degradation of a bearing results changes in the vibration signal which can be a indicator of bearing health. 
...
Once the features have been calculated one must next challenge is to isolate the most useful and relevant features. 
...
Equation \ref{eq:reqression} is used for multivariate models as well.
...
Figure \ref{fig:FeatureVsFeatureCombinedSignal9} shows the features of signal 9 plotted against each other.
\begin{figure}[H]
    \centering
...
Figure \ref{fig:SignalVsSignalFeatureFreq3} shows 
\begin{figure}[H]
    \centering
