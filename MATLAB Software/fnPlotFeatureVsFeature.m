function [R2] = fnPlotFeatureVsFeature(FeatureTable, lowerBound, upperBound, signalNum, sensorNames, featureNames, signalName)
    figure();
    t = tiledlayout(13,13,'TileSpacing','None', 'Padding','tight');
    title(t, "Signal " + signalNum + ": " + sensorNames(signalNum));
    numpoints = size(FeatureTable.(signalName(signalNum) + featureNames(1)), 1);
    ...
    for featureIndex1 = 1:numFeatures
        ...
        for featureIndex2 = featureIndex1+1 : numFeatures
            ...
            sig1 = FeatureTable.(signalName(signalNum) + feature1);
            sig2 = FeatureTable.(signalName(signalNum) + feature2);
